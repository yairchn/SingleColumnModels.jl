...
function initialize_updrafts!(q::StateVec, tmp::StateVec, grid::Grid, params, dir_tree::DirTree, ::TRMM_LBA)
  gm, en, ud, sd, al = allcombinations(q)
  @unpack params SurfaceModel
  k_1 = first_interior(grid, Zmin())
  n_updrafts = length(ud)
...
function init_state_vecs!(q::StateVec, tmp::StateVec, grid::Grid, params, dir_tree::DirTree, case::TRMM_LBA)
  @unpack params a_bounds SurfaceModel param_set
  qtg = SurfaceModel.q_tot
  Tg = SurfaceModel.T
  Pg = SurfaceModel.P
...
function init_forcing!(q::StateVec, tmp::StateVec, grid::Grid{FT}, params, dir_tree::DirTree, case::TRMM_LBA) where FT
  gm, en, ud, sd, al = allcombinations(q)
  @unpack params param_set
  z = grid.zc
  for k in over_elems_real(grid)
