...
function compute_subdomain_statistics!(grid::Grid{FT}, i, q, tmp, params, model::SubdomainMean) where {FT,IT}
...
function compute_subdomain_statistics!(grid::Grid{FT}, i, q, tmp, params, model::GaussianQuadrature) where {FT,IT}
  gm, en, ud, sd, al = allcombinations(q)
  ...
  @inbounds for k in over_elems_real(grid)
    if (q[:cv_q_tot, k, i] > eps(FT) && q[:cv_θ_liq, k, i] > eps(FT) && fabs(q[:cv_θ_liq_q_tot, k, i]) > eps(FT)
        && q[:q_tot, k, i] > eps(FT) && sqrt(q[:cv_q_tot, k, i]) < q[:q_tot, k, i])
      σ_q = sqrt(q[:cv_q_tot, k, i])
      σ_h = sqrt(q[:cv_θ_liq, k, i])
      corr = max(min(q[:cv_θ_liq_q_tot, k, i]/max(σ_h*σ_q, 1e-13),1),-1)
...
function compute_subdomain_statistics!(grid::Grid{FT}, i, q, tmp, params, model::LogNormalQuadrature) where {FT,IT}
  gm, en, ud, sd, al = allcombinations(q)
  abscissas, weights = gausshermite(model.order)
  sqrt2 = sqrt(2.0)
  sqpi_inv = 1/sqrt(pi)
  ...
  @inbounds for k in over_elems_real(grid)
    if (q[:cv_q_tot, k, i] > eps(FT) && q[:cv_θ_liq, k, i] > eps(FT) && fabs(q[:cv_θ_liq_q_tot, k, i]) > eps(FT)
        && q[:q_tot, k, i] > eps(FT) && sqrt(q[:cv_q_tot, k, i]) < q[:q_tot, k, i])
      σ_q = sqrt(log(q[:cv_q_tot, k, i]/q[:q_tot, k,i]/q[:q_tot, k,i])+1)
      σ_h = sqrt(log(q[:cv_θ_liq, k, i]/q[:θ_liq, k,i]/q[:θ_liq, k,i])+1)
      # Enforce Schwarz's inequality
      corr = max(min(q[:cv_θ_liq_q_tot, k, i]/max(σ_h*σ_q, 1e-13),1),-1)
      sd2_hq = log(corr*sqrt(q[:cv_q_tot, k, i]*q[:cv_θ_liq, k, i])/(q[:θl,k,i]*q[:q_tot,k,i]) + 1.0)
      σ_cond_θ_q = sqrt(max(σ_h*σ_h - sd2_hq*sd2_hq/σ_q/σ_q, 0.0))
      μ_q = log(q[:q_tot,k,i]*q[:q_tot,k,i]/sqrt(q[:q_tot,k,i]*q[:q_tot,k,i] + q[:cv_q_tot, k, i]))
      μ_θ = log(q[:θ_liq, k,i]*q[:θ_liq, k,i]/sqrt(q[:θ_liq, k,i]*q[:θ_liq, k,i] + q[:cv_θ_liq, k, i]))
      # clean outer vectors
      outer_src = 0.0*outer_src
      outer_env = 0.0*outer_env
